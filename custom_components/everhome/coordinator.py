...
class EverhomeDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _get_devices(self):
        ...
        async with self.auth.aiohttp_session.get(
            ...
            _LOGGER.warning("DEBUG: Raw API response: %s", devices)
            ...
            for device in devices:
                if device.get("subtype") in ["shutter", "blind", "awning", "curtain"]:
                    ...
                    _LOGGER.warning("DEBUG: Added shutter device: %s -> %s", device["id"], device)
    ...
    async def execute_device_action(self, device_id, action):
        ...
        try:
            async with self.auth.aiohttp_session.post(url, headers=headers, json=data) as resp:
                if resp.status != 200:
                    _LOGGER.error(
                        "Failed to execute action %s on device %s: %s",
                        action, device_id, await resp.text()
                    )
                    return False
                return True
        except (ClientError, asyncio.TimeoutError) as err:
